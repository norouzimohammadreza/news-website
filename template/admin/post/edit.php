...
<section class="row my-3">
    <section class="col-12">
        ...
        <form method="post" action="<?= url('admin/post/update' . '/' . $post['id']); ?>" enctype="multipart/form-data">
                ...
                <div class="form-group">
                   ...
                   <?php foreach ($categories as $category) { ?>
                        <option value=" <?= $category['id'] ?>" <?php if($category['id']==$post['cat_id'])echo 'selected';?>> <?= $category['title']   ?></option>
                  <?php } ?>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="published_at_view" value="<?= $post['published_time']; ?>" required autofocus>
        </div>
